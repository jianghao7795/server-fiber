...
type Comment struct {
	...
	ParentId  int            `json:"parent_id" form:"parent_id" query:"parent_id" gorm:"column:parent_id;comment:上级;size:10;"`
	Content   string         `json:"content" form:"content" gorm:"column:content;comment:内容;"`
	UserId    int            `json:"user_id" form:"user_id" query:"user_id" gorm:"column:user_id;comment:用户id;"` // 作为外键 关联User
	...
	ToUser    system.SysUser `json:"to_user" form:"to_user" query:"to_user" gorm:"foreignKey:ToUserId"`
	ToUserId  int            `json:"to_user_id" form:"to_user_id" query:"to_user_id" gorm:"column:to_user_id;comment:通知用户id;"` // 作为外键 关联User
}
